class TaquillaPage {
  constructor(page) {
    ...
    this.listaPeliculas = page.locator("ul li.ipc-metadata-list-summary-item");

    // Botón Puntuar
    this.botonCalificacion = page
      .locator('[data-testid="hero-rating-bar__user-rating__unrated"]')
      .first();

    // Barra de estrellas del modal
    this.barraEstrellas = page.locator("div.ipc-starbar__touch");
  }
  ...
  async navegarATaquillaSuperior() {
    ...
    await this.categoriaPeliculas.evaluate((element) => element.click());

    await this.enlaceTaquilla.click();

    // Validar navegación
    await this.page.waitForURL(/boxoffice/, {
      timeout: 15000,
    });
  }
  ...
  async abrirSegundaPelicula() {
    ...
    await segundaPelicula.locator("a").first().click();
